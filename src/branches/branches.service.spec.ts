  ...
  describe('Список и карточка', () => {
    it('отдаёт постраничный список со счётчиками привязанных записей', async () => {
      repository.findMany.mockResolvedValue({
        rows: [
          row({
            _count: { rooms: 4, groups: 7, students: 120, employees: 9, leads: 0, expenses: 0 },
          }),
        ],
        total: 1,
      });
  ...
  describe('Удаление', () => {
    ...
    it('409, если к филиалу привязаны студенты — с перечислением причин', async () => {
      repository.findById.mockResolvedValue(
        row({ _count: { rooms: 2, groups: 0, students: 15, employees: 0, leads: 0, expenses: 0 } }),
      );
    ...
    it('409, если в филиал записаны лиды (ТЗ 5.7)', async () => {
      repository.findById.mockResolvedValue(
        row({ _count: { rooms: 0, groups: 0, students: 0, employees: 0, leads: 5, expenses: 0 } }),
      );
    ...
    it('409, если к филиалу привязаны только сотрудники', async () => {
      repository.findById.mockResolvedValue(
        row({ _count: { rooms: 0, groups: 0, students: 0, employees: 3, leads: 0, expenses: 0 } }),
      );
    ...
    it('409, если в филиале набраны группы (ТЗ 5.5 — связь RESTRICT)', async () => {
      repository.findById.mockResolvedValue(
        row({ _count: { rooms: 0, groups: 4, students: 0, employees: 0, leads: 0, expenses: 0 } }),
      );
