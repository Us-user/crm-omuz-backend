...
**Общий прогресс:** Фаза 3 из 14 · 35 из 97 пунктов (~36%) — Фаза 0: 11/13 · **Фаза 1: 9/9 (закрыта)** · **Фаза 2: 8/8 (закрыта)** · Фаза 3: 7/10
...
- [~] Модели: `Branch`, `Course`, `SyllabusLesson`, `ResourceFile`, `Room`, `Group`, `GroupMentor`, `ScheduleSlot`, `GroupStudent`, `Enrollment` — готовы `Branch`, `Room`, `Course`, `Group`, `SyllabusLesson`, `ResourceFile`, `GroupMentor`, `ScheduleSlot` (+ `SyllabusLessonGroup` под «Show to group», + `Student.branchId`/`Employee.branchId` по ТЗ 3.3); состав студентов — следующим куском
