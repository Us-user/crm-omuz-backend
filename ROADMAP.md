...
**Общий прогресс:** Фаза 5 из 14 · 46 из 97 пунктов (~47%) — Фаза 0: 11/13 · **Фаза 1: 9/9 (закрыта)** · **Фаза 2: 8/8 (закрыта)** · Фаза 3: 9/10 (последний пункт — категории активности — ждёт общего балла) · Фаза 4: 4/7 закрыто плюс два `[~]` (фильтр «Contract» ждёт модели договора, корона и разделы кабинета — общего балла) · Фаза 5: 5/7 закрыто плюс `[~]` на моделях (нет `Performance`); открыт пункт «общий балл → рейтинг/корона/категории», на котором висят остатки Фаз 3 и 4
...
- [~] Фильтры (name/Group/Course/Status/Contract), топ-студент (корона) — сделаны `search` (имя/фамилия/телефон/email), `groupId`, `courseId`, `status` + сверх ТЗ `branchId` и `hasAccount` (кого пригласить); **фильтр Contract** ждёт модели договора, **корона** — общего балла (пункт 4 Фазы 5: журнал уже есть, среднее по неделям ещё не считается)
