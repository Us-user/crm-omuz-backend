...
**Общий прогресс:** Фаза 4 из 14 · 41 из 97 пунктов (~42%) — Фаза 0: 11/13 · **Фаза 1: 9/9 (закрыта)** · **Фаза 2: 8/8 (закрыта)** · Фаза 3: 9/10 (последний пункт ждёт журнала Фазы 5) · Фаза 4: 4/7 закрыто плюс два `[~]` (фильтр «Contract» и корона, кабинет студента без баллов и рейтинга) — обе половины ждут Фазы 5
...
- [~] Фильтры (name/Group/Course/Status/Contract), топ-студент (корона) — сделаны `search` (имя/фамилия/телефон/email), `groupId`, `courseId`, `status` + сверх ТЗ `branchId` и `hasAccount` (кого пригласить); **фильтр Contract** ждёт модели договора, **корона** — среднего балла из Фазы 5
