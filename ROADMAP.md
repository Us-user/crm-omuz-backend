...
**Общий прогресс:** Фаза 3 из 14 · 36 из 97 пунктов (~37%) — Фаза 0: 11/13 · **Фаза 1: 9/9 (закрыта)** · **Фаза 2: 8/8 (закрыта)** · Фаза 3: 8/10
...
- [~] Состав студентов: add/transfer/change-status(с Reason)/import/export (`/groups/{id}/students...`) — готовы зачисление пачкой, `transfer`, `change-status` с обязательным Reason и исключение из состава; **импорт/экспорт (CSV) — следующим куском**. Членство со своим статусом (`ACTIVE`/`LEFT`/`FINISHED`/`TRANSFERRED`), а не `Student.status` (решение сессии 0012); одно действующее членство на курс (409), закрытые не мешают; перевод закрывает прежнюю строку и заводит новую одной транзакцией. Заодно закрыты «Required students = набрано/вместимость» (`enrolledCount`) и «группу с составом не удалить» (409, обещание сессий 0008–0009)
