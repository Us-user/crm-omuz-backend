...
- **филиал с привязанными записями не удаляется** (409): аудитории, студенты и сотрудники
  ссылаются на него `RESTRICT`. Чтобы вывести филиал из работы, не теряя историю, —
  статус `INACTIVE`;
- названия филиала и курса уникальны **без учёта регистра**; название аудитории — уникально
  **внутри филиала** («101» может быть в каждом);
- несуществующий `branchId` в теле запроса — **422**, а не 404: ресурс из пути найден,
  не найдено то, что пришло в теле (тот же выбор, что в назначении ролей);
...
- пустая строка в необязательном текстовом поле `PUT` **очищает** его, а не переданное
