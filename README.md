...
`JWT_ACCESS_SECRET` и `JWT_REFRESH_SECRET` обязательны, минимум 32 символа и **разные**:
access-токен, утёкший в логи прокси, не должен приниматься как refresh. Сгенерировать:
...
```
src/
  auth/        регистрация, вход, JWT с ротацией, серверные сессии, глобальный guard
