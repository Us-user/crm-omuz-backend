...
Доступ пока ограничен грубо — декоратором `@RequireAccountType(AccountType.EMPLOYEE)`
и `AccountTypeGuard`: полномочия студента ограничены просмотром своих данных (ТЗ 3.2),
поэтому повысить себя он не может. Проверка по каталогу прав придёт в Фазе 2.
