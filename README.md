...
- **E2E** (`npm run test:e2e`) — в `test/`:
  - `api-conventions.e2e-spec.ts`, `health-route.e2e-spec.ts`, `auth.e2e-spec.ts`,
    ...
    `syllabus.e2e-spec.ts`, `group-mentors.e2e-spec.ts` и `group-schedule.e2e-spec.ts`
    работают **без** PostgreSQL и Redis:
    ...
    по этому адресу не найдётся»), а также расписания («аудитория занята другой группой»,
    «аудиторию с занятиями не удалить») проверяются на любой машине);
